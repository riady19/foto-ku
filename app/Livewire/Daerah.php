...
class Daerah extends Component
{
    ...
    {
         $this->artikel = Artikel::where('catagory_id', '6')->where('catagory_id', 6)->where('status', 'publish')->limit(1)->orderBy('created_at', 'desc')->get()->map(function($item){$item->tanggal_published = Carbon::parse($item->updated_at)->locale('id')->translatedFormat('l, d F Y');
