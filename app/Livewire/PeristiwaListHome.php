...
class PeristiwaListHome extends Component
{
    public $artikel, $catagory_id, $tranding;
    public function mount()
    {
         $this->artikel = Artikel::where('catagory_id', 5)->where('status', 'publish')->limit(4)->orderBy('created_at', 'desc')->offset(1)->get()->map(function($item){$item->tanggal_published = Carbon::parse($item->updated_at)->locale('id')->translatedFormat('l, d F Y');
    ...
    {
        return view('livewire.peristiwa-list-home');
    }
}
