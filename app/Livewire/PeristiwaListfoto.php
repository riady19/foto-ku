...
class PeristiwaListfoto extends Component
{
    public $artikel, $catagory_id, $tranding;
    public function mount()
    {
         $this->artikel = Artikel::where('catagory_id', '3')->where('catagory_id', 3)->where('status', 'publish')->limit(3)->orderBy('created_at', 'desc')->offset(1)->get()->map(function($item){$item->tanggal_published = Carbon::parse($item->updated_at)->locale('id')->translatedFormat('l, d F Y');
    ...
    {
        return view('livewire.peristiwa-listfoto');
    }
}
